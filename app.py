...
def bill_detail_panel(row):
    ...
    with c1:
        ...
        if pd.notna(row.get("total_kwh_equivalent")):
            st.markdown(f"**Total GHG usage:** {float(row['total_kwh_equivalent']):,.0f} kWh equiv.")
...
with st.sidebar:
    ...
    with st.expander("📂 Report-27 — Bill Flags (required)", expanded=True):
        r27_files = st.file_uploader(
            "One or more Report-27 exports", type=["xlsx"],
            accept_multiple_files=True, key="r27_upload",
    ...
    with st.expander("📂 Report-18 — Bill Line Items (optional)", expanded=True):
        r18_files = st.file_uploader(
            "One or more Report-18 exports (Use sheet)", type=["xlsx"],
            accept_multiple_files=True, key="r18_upload",
    ...
    if not df_all_ss.empty:
        ...
        vendor_filter   = st.multiselect("Vendor", _all_v, default=_all_v)
...
with t_overview:
    ...
    with col1:
        ...
        colors_i = [PRIORITY_COLOR.get(FLAG_META.get(k,{}).get("priority","Medium"),"#6c757d")
                    for k in top12]
        fig_iss = go.Figure(go.Bar(x=list(top12.values()),y=list(top12.keys()),
...
with t_ghg:
    ...
    if not has_usage:
        st.info("Upload **Report-18** (Bill Line Item Report, Use sheet) to enable GHG analysis.")
    else:
        ...
        if ghg_df.empty:
            ...
            cat_summary.columns = ["GHG Category","Bills","Total Usage","Unresolved"]
            ...
            if cat_bar:
                sc_colors = {"Electricity":"#0d6efd","Natural Gas":"#fd7e14",
                             "LPG / Propane":"#e67e22","Biomass":"#198754",
                             "District Heat / Steam":"#6f42c1",
                             "Diesel / Fuel Oil":"#dc3545"}
                fig_cat = go.Figure(go.Bar(
                    x=list(cat_bar.values()),y=list(cat_bar.keys()),orientation="h",
                    marker_color=[sc_colors.get(k,"#888") for k in cat_bar],
                    text=[f"{v:,.0f}" for v in cat_bar.values()],textposition="outside"))
                fig_cat.update_layout(
                    title=f"Usage by commodity ({disp_unit})",yaxis_autorange="reversed",
                    xaxis=dict(showgrid=True,gridcolor="#f0f0f0"),
            ...
            pq = ghg_df[["bill_id","vendor","billing_period_label","status",
                         "ghg_category","ghg_scope","display_usage","cost",
                         "primary_priority","flag_issues","assigned_to","priority_score"]].copy()
            pq.columns = ["Bill ID","Vendor","Period","Status","Commodity","Scope",
